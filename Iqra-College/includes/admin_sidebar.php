 ...
 * Include in admin pages for consistent navigation
 */
...
$adminNavMain = [
    ...
    ['href' => 'teachers.php', 'icon' => 'fa-chalkboard-teacher', 'label' => 'Teachers & Cashiers'],
    ['href' => 'courses.php', 'icon' => 'fa-book', 'label' => 'Courses'],
    ['href' => 'categories.php', 'icon' => 'fa-folder', 'label' => 'Categories'],
    ['href' => 'students.php', 'icon' => 'fa-user-graduate', 'label' => 'Students'],
];
$adminNavAccess = [
    ['href' => 'assign-teachers.php', 'icon' => 'fa-link', 'label' => 'Assign Teachers'],
    ['href' => 'assign-students.php', 'icon' => 'fa-user-tag', 'label' => 'Assign Students'],
    ['href' => 'manage-users.php', 'icon' => 'fa-users-cog', 'label' => 'Manage Users'],
];
$adminNavConfig = [
    ['href' => 'events.php', 'icon' => 'fa-calendar-check', 'label' => 'Events'],
    ['href' => 'voice-call-config.php', 'icon' => 'fa-phone', 'label' => 'Voice Call Config'],
];
?>
<style>
.sidebar-link { transition: all 0.3s ease; }
.sidebar-link:hover { background: linear-gradient(90deg, rgba(99, 102, 241, 0.1), rgba(139, 92, 246, 0.1)); transform: translateX(5px); }
.sidebar-link.active { background: linear-gradient(90deg, rgba(99, 102, 241, 0.2), rgba(139, 92, 246, 0.2)); border-left: 4px solid #6366f1; font-weight: bold; }
    ...
    <div class="p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900">
        <div class="flex items-center space-x-3">
            <?php if (!empty($userAvatar) && file_exists(__DIR__ . '/../uploads/avatars/' . $userAvatar)): ?>
                <img src="../uploads/avatars/<?php echo htmlspecialchars($userAvatar); ?>" alt="Profile" class="w-14 h-14 rounded-2xl object-cover border-2 border-indigo-500 shadow-lg">
            <?php else: ?>
                <div class="gradient-bg w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wide">Admin</p>
            </div>
        </div>
    </div>

    <nav class="flex-1 overflow-y-auto p-4 space-y-1">
        <?php foreach ($adminNavMain as $item): ?>
        <a href="<?php echo htmlspecialchars($item['href']); ?>" class="sidebar-link <?php echo $currentPage === $item['href'] ? 'active' : ''; ?> flex items-center space-x-3 p-3 rounded-lg text-gray-700 dark:text-gray-300">
            <i class="fas <?php echo $item['icon']; ?> text-indigo-600 dark:text-indigo-400 w-5"></i>
        ...
        <p class="px-3 py-1 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Access Control</p>
        <?php foreach ($adminNavAccess as $item): ?>
        <a href="<?php echo htmlspecialchars($item['href']); ?>" class="sidebar-link <?php echo $currentPage === $item['href'] ? 'active' : ''; ?> flex items-center space-x-3 p-3 rounded-lg text-gray-700 dark:text-gray-300">
            <i class="fas <?php echo $item['icon']; ?> text-indigo-600 dark:text-indigo-400 w-5"></i>
            <span class="font-medium"><?php echo htmlspecialchars($item['label']); ?></span>
        </a>
        ...
        <div class="my-3 border-t border-gray-200 dark:border-gray-600"></div>
        <p class="px-3 py-1 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Settings</p>
        <?php foreach ($adminNavConfig as $item): ?>
        <a href="<?php echo htmlspecialchars($item['href']); ?>" class="sidebar-link <?php echo $currentPage === $item['href'] ? 'active' : ''; ?> flex items-center space-x-3 p-3 rounded-lg text-gray-700 dark:text-gray-300">
            <i class="fas <?php echo $item['icon']; ?> text-indigo-600 dark:text-indigo-400 w-5"></i>
            <span class="font-medium"><?php echo htmlspecialchars($item['label']); ?></span>
        </a>
...
<aside id="mobile-sidebar" class="lg:hidden fixed left-0 top-0 h-full w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 z-40 flex flex-col -translate-x-full transition-transform duration-300">
    <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        <div class="flex items-center space-x-3">
            <?php if (!empty($userAvatar) && file_exists(__DIR__ . '/../uploads/avatars/' . $userAvatar)): ?>
                <img src="../uploads/avatars/<?php echo htmlspecialchars($userAvatar); ?>" alt="Profile" class="w-12 h-12 rounded-2xl object-cover border-2 border-indigo-500">
            <?php else: ?>
                <div class="gradient-bg w-12 h-12 rounded-2xl flex items-center justify-center">
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-1">
        <?php foreach (array_merge($adminNavMain, $adminNavAccess, $adminNavConfig) as $item): ?>
        <a href="<?php echo htmlspecialchars($item['href']); ?>" class="sidebar-link <?php echo $currentPage === $item['href'] ? 'active' : ''; ?> flex items-center space-x-3 p-3 rounded-lg text-gray-700 dark:text-gray-300">
            <i class="fas <?php echo $item['icon']; ?> text-indigo-600 dark:text-indigo-400 w-5"></i>
            <span class="font-medium"><?php echo htmlspecialchars($item['label']); ?></span>
        </a>
